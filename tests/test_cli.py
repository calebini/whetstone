...
class CliTests(unittest.TestCase):
    def test_resume_help_includes_recovery_examples(self) -> None:
        ...
        self.assertIn("Phase 1 Reviewer/Editor timeouts", rendered)
        self.assertIn("Phase 1 Editor artifact-validation retries", rendered)
        self.assertIn("explicit Phase 1 budget extension", rendered)
    ...
    def test_decompose_plan_inventory_writes_no_split_artifacts(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["decomposition_map_template"], str(output_dir / "decomposition_map_template.json"))
            ...
            template = json.loads(output_dir.joinpath("decomposition_map_template.json").read_text(encoding="utf-8"))
            self.assertEqual(template["template_kind"], "decomposition_map_template")
            self.assertEqual(template["source_spec_hash"], plan["source_spec_hash"])
            self.assertEqual(template["planning_mode"], "proposed_split")
            self.assertIn("coordinated_family", template["authority_topology_options"])
            self.assertIn("leaf_spec", template["target_spec_role_options"])
            self.assertEqual(len(template["extractable_units"]), 2)
            self.assertEqual(template["target_specs"], [])
            self.assertEqual(template["target_spec_template"]["source_units"][0]["scope"], "section")
