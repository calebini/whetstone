...
class LiveRoundRunnerTests(unittest.TestCase):
    def test_live_round_writes_guarded_packet_without_mutating_spec(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            profile_used = json.loads(round_dir.joinpath("profile_used.yaml").read_text(encoding="utf-8"))
            self.assertEqual(profile_used["profile"], "convergence_strict_check")
            self.assertEqual(profile_used["round_kind"], "review_editor")
