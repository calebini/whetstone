...
The [current-runtime preservation bridge](specs/CANDIDATE_EDITING_AND_PROMOTION_SPEC.md#current-runtime-preservation-bridge) has implemented schema/inventory foundations and developer components for proposal capture, preliminary assessment and isolated acceptance/repair, plus scripted horizontal/vertical runtime, continuation, maintenance and closeout integration. Public runtime activation is still unavailable. Current full-draft editing is **not** made non-destructive by a bounded prompt, scope contract, or the CLI's historical "guarded" wording. For valuable drafts, keep an isolated source copy and use reviewer-only assessment when automatic rewriting is not acceptable.

The reserved `preservation_bridge` configuration in the [coordinating spec](specs/WHETSTONE_COORDINATING_SPEC.md#preservation-bridge-activation) is not a runnable option today; supplying it fails configuration loading with `CONFIG_INVALID`. The [developer guide](../contracts/PRESERVATION_BRIDGE.md) describes the available contract checks and proposal-capture library; isolated Phase 1 roots now have [local acceptance developer commands](PRESERVATION_ACCEPTANCE_DEVELOPER.md), and the [runtime developer checkpoint](PRESERVATION_RUNTIME_DEVELOPER.md) exercises ordinary/focused Phase 1, local completion, typed Reviewer/Editor retry, horizontal scheduler continuation, review-only verification, vertical cycles and closeout recovery, and consumer guards. Public live enforcement remains unavailable. Once public opt-in activation ships, the workflow is:
...
Missing effect evidence at proposal preparation maps to `PAUSED_DECISION`, not malformed Editor output. Known hard violations still reject. The [acceptance request contract](specs/ARTIFACTS_VALIDATION_AND_TELEMETRY_SPEC.md#acceptance-request-and-admission) now has isolated developer operations: `preservation-review`, `preservation-attest`, `preservation-request`, and `preservation-accept --dry-run`. See the linked developer guide for full syntax. The developer runtime now supports Phase 2 entry maintenance, inherited no-op authorization and guarded Phase 2 rounds. The developer runtime also supports explicitly admitted Phase 1 budget extensions and bounded Phase 2 `closeout-existing` recovery. The [qualification record](PRESERVATION_QUALIFICATION.md) covers line-based effect adoption, the CLI journey, exact-byte apply-back and verification of convergence from retained evidence. Scripted cross-path qualification is complete; public activation stays unavailable pending the separate release change.
