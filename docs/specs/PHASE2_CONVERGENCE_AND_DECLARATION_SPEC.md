...
Phase 2 entry version promotion uses the artifact leaf's narrowly defined `phase2_entry` proposal and acceptance admissions over accepted bytes. The Orchestrator may complete this trusted maintenance path with empty operator evidence; it must still persist both admissions and final acceptance. Preserve its raw base, materialized output and ordered trusted transform, and validate/persist acceptance before writing the first Phase 2 draft. Unversioned entry remains a no-op. Neither promotion nor reviewer-only closeout creates edit authorization for the promoted draft: a later Editor requires an approved surface bound to that exact new base.

Every Phase 2 Editor/fixture path uses the same two-stage bridge as Phase 1. An exact-effect evidence pause uses `PAUSED_DECISION`; the explicit local acceptance operation may finalize retained output in the same root without an Editor call or generic Phase 2 timeout resume. Ordinary phase/profile/declaration conditions still apply. Declaration acceptance additionally binds the final accepted bridge marker and exact draft hash; neither a preliminary proposal report nor operator approval alone can satisfy it. Validate the complete proposal, request, evidence, final report and acceptance-marker chain. Rejected/paused/incomplete materialized bytes are excluded regardless of exhausted budgets or a clean profile result. Reviewer-only clean rounds can verify the already accepted current draft without inventing an Editor mutation. A valid no-op needs no version increment.
