...
### Two-Stage Proposal And Acceptance

Before Editor invocation, proposal admission freezes exact base/inventory, approved scope, admitted findings, allowed/frozen surface and materialization policy. This authorizes bounded proposal generation, never mutation of authority. It does not require attestations over output bytes that do not yet exist.

Persist raw and materialized output, inventories, normal-round evidence and an immutable proposal record before assessing exact effects. A preliminary assessment exposes complete known changes and outstanding correspondence/attestation obligations. Missing operator evidence alone is expected pending work, not malformed Editor output and not a reason to reinvoke the model. Known hard failures remain rejection. An operator may inspect grouped effects and adopt exact evidence through the trusted local channel.

A distinct acceptance admission binds the retained proposal to explicit approved acceptance scope/surface/findings and operator evidence. It invokes no Editor and cannot modify proposal bytes or materialization. All preservation and ordinary acceptance checks rerun before a marker can commit. Changed authorization requires a new immutable acceptance admission; changed content or a stale base requires a new proposal. Same-root admission is permitted under the scheduler's guards. No preliminary report or approval by itself advances authority.
...
Correspondence MUST NOT use fuzzy similarity or an Editor's identity claim. First match identical direct-content sequences inside unchanged canonical section paths; for edited sections, retain exact line matches only when there is one unambiguous order-preserving match. Equal-content duplicates with several possible predecessors remain ambiguous. Explicit operator evidence may supply an exact predecessor/successor mapping for rewording, moves, renames and supersession. The [scope leaf](SCOPE_INTAKE_AND_DECISIONS_SPEC.md#explicit-correspondence-rules) defines explicit `{base_unit_id, successor_unit_ids, disposition}` entries, one-to-many reflow and complete many-to-one supersession groups. Array position never implies pairing. Validate adopted mappings against both inventories. Missing resolvable correspondence is pending during proposal assessment; conflicting supplied mappings are a hard failure, and incomplete correspondence always blocks acceptance. A rename of a container MUST account for all descendant units, not only its heading. Every output unit must ultimately be a successor or explicit addition, never both; preliminary unmapped output is reported without guessing.
...
For `preserved`, exact content and structural ownership MUST remain equal, except a recorded trusted version transformation. Changed content cannot pass as preserved merely because normalized draft hashes match. For changed wording, strengthening, movement or supersession, first-release admissible evidence is an explicit hash-bound operator attestation defined in the scope leaf. During proposal assessment, absent evidence is an explicit pending obligation when no independent hard failure exists. During submitted acceptance, absent evidence requires rejection with `ambiguous`; unsupported supersession additionally records `unauthenticated_supersession` in `failures.category`, never as a unit disposition. A clean Reviewer pass, Editor assertion, line-count ratio or generic similarity score is not an attestation. This conservative bridge validates evidence bindings, not the truth of arbitrary natural-language equivalence; reports MUST identify operator-attested claims as such. A future independent verifier needs a separately versioned admission rule.

The bridge MUST block acceptance of unrelated contract loss, unresolved protected-unit correspondence, unauthorized deletion/weakening, surface overruns, and concrete contracts replaced by summaries or placeholders without individually valid dispositions. Pending correspondence or semantic attestation does not conceal a known hard violation. Legitimate removal is possible through explicit deletion or evidenced supersession; preservation does not mean retaining all text forever.
...
Every attempt retains immutable inputs and available outputs. Every completed assessment, passing, pending or failing, retains an immutable report identifying its proposal or acceptance stage. A full-draft rewrite attempt is evidence, not a vNext candidate, and a bridge pass alone is not normal round acceptance. Failed/pending bytes MUST NOT become the next base, satisfy issues, erase residuals or pass recovery/strop controls. New authorization never rewrites a failed report; use the scheduler's explicit new-admission procedure. Transient technical continuation retains frozen bindings, whereas deterministic violations receive no automatic retry.
...
Qualification MUST also demonstrate the complete operator journey, not only rejection correctness:

- An ordinary clarification: approved bounded surface -> retained proposal -> pending equivalence -> explicit one-to-one evidence -> same-root acceptance with zero further Editor calls.
- A harmless addition follows the same path with addition evidence; a disguised exception also requires weakening authorization for affected retained units.
- One-to-many reflow and many-to-one legitimate consolidation pass with complete explicit correspondence; swapped pairings, conflicting entries, incomplete supersession groups and uncovered output reject.
- Missing mappings are reviewed explicitly, never inferred from array position or fuzzy similarity. Grouped operator adoption must generate valid exact bindings without manual hash/JSON construction.
- A changed acceptance surface/evidence set creates a new attempt and preserves prior reports; ordinary findings/residuals cannot be erased by selecting a smaller surface.
- Approval followed by base drift refuses; transformed/raw tampering and a different required stamp refuse. Corruption or a known forbidden mutation cannot pass through pending approval.
- Repeated acceptance, including accepted no-ops, and crashes before/after commit create no duplicate authority/history changes. Technical retry never refreshes authorization.
- Supplied revisions, both phases, vertical editing, no-ops, Phase 2 maintenance and strop all consume only committed acceptance evidence. Generic Phase 2 technical resume remains unsupported.

Each positive fixture MUST document the displayed effect, required approvals, operator interaction count, proposal/acceptance artifacts and model-call count. Controlled live usability evidence may follow separately authorized trials; a passing rejection suite alone is insufficient qualification.
