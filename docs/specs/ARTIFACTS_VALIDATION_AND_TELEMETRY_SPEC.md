...
These are public specification contracts for the pending `preservation-bridge-v1` capability. Closed schema files, structural validators and developer components for immutable proposal admission/capture, trusted materialization and preliminary assessment are implemented. Isolated local acceptance admission, effect review, marker commit, repair and request/dry-run commands are also implemented; runtime integration and full qualification remain pending. They are independent of the vNext contract suite. The two-stage amendment supersedes the earlier unpublished single-admission draft. It retains the reserved capability name but requires the proposal/acceptance contracts below, `bridge-operator-evidence-v2`, `current-runtime-preservation-bridge-report-v2`, and `preservation-bridge-acceptance-v2` as one qualified implementation. Earlier draft admission/evidence/report/acceptance shapes MUST NOT be accepted as these contracts; this is not a migration or a runtime release. The capability MUST NOT be advertised until equivalent closed schemas, validators and conformance fixtures ship. All declared fields are required unless explicitly optional; unknown fields fail, arrays are ordered and duplicate-free, integers exclude booleans, and hashes are 64 lowercase hexadecimal SHA-256 characters. `Ref` means exactly `{path: string, sha256: sha256}` over persisted bytes. Paths MUST be relative to the run root, resolve within it without symlink escape, and identify regular files. Missing files, changed bytes or unsupported schema/algorithm versions fail closed. Client output never owns these hashes.
...
The bridge has separate proposal and acceptance admissions. Proposal generation freezes edit authority before any Editor invocation; exact-effect operator evidence is obtained only after immutable output exists. An acceptance admission never invokes an Editor. All guarded paths, including supplied revisions and maintenance no-ops, use these two stages; maintenance may complete both stages without human effect review under the narrow rules below.

Allocate a positive monotonic proposal attempt number `M` within round `N`, including technical retries, supplied revisions and no-ops. Before invocation create `rounds/round-N/preservation/attempt-M/admission.json` and immutable input snapshots. Never reuse an incomplete attempt number. This number identifies a proposal attempt, not a client retry or vNext candidate. Within that proposal directory allocate a separate positive monotonic acceptance attempt number `K` under `acceptance-attempt-K/`. Multiple acceptance attempts may reference one proposal; they MUST NOT overwrite each other. Phase 2 entry uses the special proposal directory defined below.

Proposal admission schema `preservation-bridge-proposal-admission-v1` contains exactly `schema_version`, `capability_version = preservation-bridge-v1`, `round_number` (positive integer or null), `attempt_number` (positive integer), `phase` (`phase_1 | phase_2`), `profile` (nonempty string or null), `origin` (`editor | supplied_revision | orchestrator_noop | phase2_entry`), nullable positive `client_attempt_number`, `base_draft`, `inventory`, `scope_contract`, `allowed_change_surface` (all Refs), `finding_sources` (manifest records), nullable `predecessor_report: Ref`, and `transform_policy_version = bridge-version-transform-v1`. `round_number` and `profile` MUST both be null if and only if `origin = phase2_entry`. `client_attempt_number` is positive only for `editor`, otherwise null. Inputs are immutable snapshots. No operator-effect evidence or future proposal/output hash is a proposal-admission prerequisite. Validate the approved surface and scope before invoking an Editor; omission of effect evidence does not permit an unbounded proposal.

Persist the original client response through existing attempt storage. Strictly JSON-decode `draft_after_content`; encode that string as UTF-8 without newline, whitespace, Unicode or replacement normalization into `rounds/round-N/full_draft_rewrite_attempt-M.md`. This raw draft differs from its response envelope. Invalid JSON/Unicode may leave only response evidence. Supplied revisions retain exact file bytes; maintenance raw proposals equal base bytes. Retain validated Reviewer artifacts and the validated Editor summary, or the existing Orchestrator-owned fixture/no-op summary, as immutable normal-round evidence. No proposal-stage summary or claimed issue resolution becomes canonical accepted state.
...
Materialization computes the prospective revision stamp from the frozen base and normal round evidence before operator effect review. Only a round otherwise eligible under ordinary issue acceptance receives that prospective stamp. No materialized proposal advances the authoritative version or history before acceptance commit, even when a prospective stamp exists. Acceptance MUST reproduce the same transformation; changed transformation inputs or a different required stamp require a new proposal, not post-approval restamping. Recompute all transform outputs from immutable inputs under the pinned implementation; do not apply them twice on resume. Materialized inventories, preservation checks and canonical Editor summary hashes are computed only from the final bytes. The compatibility `draft_hash` is separately recorded using the existing normalization algorithm.

After materialization, persist `proposal.json` in the proposal directory with schema `preservation-bridge-proposal-v1` and exactly `schema_version`, `admission: Ref`, `raw_response: Ref | null`, `raw_proposal: Ref`, `materialized_draft: Ref`, `materialized_inventory: Ref`, `materialized_draft_hash: sha256` (normalized compatibility hash), `transformations` (the ordered records defined in the report below), and `normal_round_evidence: {effective_config: Ref, reviewer_feedback: [Ref], editor_summary: Ref | null}`. The materialized inventory uses its own exact draft Ref. Validate normal-round evidence against the proposal's base, origin, phase, profile and findings. The effective-config snapshot MUST be retained before proposal execution; its Ref freezes the existing resolved phase/workflow/profile/budget settings for completing this operation; acceptance must not silently apply later defaults. For `phase2_entry`, reviewer feedback is empty and the summary is null; ordinary Phase 1 handoff checks supply its maintenance eligibility, rather than a fabricated null-round Editor summary. For every other origin, a validated non-null summary is required. `raw_response` is non-null for `editor` and null for other origins. The proposal Ref identifies these immutable bytes; it never includes later reports, operator evidence, acceptance requests or admissions. If materialization cannot complete, no complete proposal record is fabricated.

Phase 2 entry uses `rounds/preservation/phase2-entry/attempt-M/` with `admission.json`, `raw_proposal.md`, `materialized_draft.md`, `materialized_inventory.json`, `proposal.json`, `report.json`, and `acceptance-attempt-K/`. Its round/profile/client attempt fields are null. It inherits the last accepted bridge authorization for provenance and permits only the specified version/status transform over accepted bytes. Reviewer-only Orchestrator no-ops may likewise inherit prior authorization with a newly generated inventory. Only these maintenance origins may reference an older surface/base through a validated acceptance chain. They do not authorize an Editor or other content change. Initial seed no-ops without a prior marker require normal admission. An unchanged no-op needs no increment or new effect attestation; a verified Phase 2 entry transform likewise needs no semantic attestation.

#### Acceptance Request And Admission

The explicit local operator channel supplies an immutable `preservation-bridge-acceptance-request-v1` with exactly `schema_version`, `proposal: Ref`, `scope_contract: Ref`, `allowed_change_surface: Ref`, `finding_sources` (manifest records), `operator_evidence: [Ref]`, and nullable `predecessor_report: Ref`. This is a contract for the pending acceptance operation, not an Editor input or an implemented CLI command. The implementation MUST expose a local request-file operation and a read-only dry-run before bridge qualification; interface documentation must describe the actual supported command when it lands. An Orchestrator-owned maintenance request uses the same shape with empty evidence and the maintenance rules above. The Orchestrator may also prepare an empty-evidence request for a genuinely unchanged proposal from any origin, retaining that origin and its normal-round evidence; no-op handling cannot resolve outstanding serious findings by itself.

The request may select newly approved scope/surface/findings for the same exact base. It cannot change content, phase, profile, origin, materialization policy or the frozen normal-round evidence. New finding sources must be validated for that base; existing ordinary round findings and residuals cannot be discarded by changing the acceptance surface. Finding attribution and ordinary acceptance are separately checked. Changed authorization must be explicit; runtime configuration edits and technical resume do not silently select a request.

After validating the request, persist `acceptance-attempt-K/admission.json` with schema `preservation-bridge-acceptance-admission-v1` and exactly `schema_version`, `capability_version = preservation-bridge-v1`, `acceptance_attempt_number: positive integer`, `request: Ref`, `proposal: Ref`, `base_draft: Ref`, `inventory: Ref`, `scope_contract: Ref`, `allowed_change_surface: Ref`, `finding_sources`, `operator_evidence: [Ref]`, nullable `predecessor_report: Ref`, and `transform_policy_version = bridge-version-transform-v1`. Every duplicated binding MUST equal its request/proposal source; base/inventory/transform derive from the proposal admission, acceptance authorization derives from the request. Recheck current authority before admission and commit. Persist immutable input references/snapshots without modifying referenced paths or approvals. An invalid request fails preflight without a fabricated admission.

Evidence MUST bind the immutable proposal and the acceptance scope/surface, not the earlier proposal authorization when it differs. Reference direction is acyclic: base/scope/findings -> surface -> proposal admission -> proposal -> operator evidence -> request -> acceptance admission -> report -> acceptance marker. Acceptance-side scope/surface snapshots may be prepared after the proposal, but cannot reference the request or evidence that binds them. Predecessor reports point only backward. No object includes its own eventual hash or a downstream artifact's hash.

A changed evidence set or authorization creates a new `K`; changed bytes or transformation inputs require a new `M`. When a prior report exists for the same proposal, an explicit new request MUST reference the immediately preceding assessment report through `predecessor_report`; omission cannot erase its provenance. First acceptance references the proposal assessment, later acceptance references the prior acceptance assessment. Incomplete attempts without a report remain listed by their admission and cannot be erased or silently reused. A completed report never changes. Same-root re-admission is governed by the scheduler; a retained rejected proposal is evidence, never the authoritative base.

### Current-Runtime Preservation Bridge Report

Proposal assessments use `report.json` beside proposal admission. Acceptance assessments use `report.json` beside acceptance admission. Both use the closed shape below. Report identity is the full Ref, not `M` or `K` alone. Every attempt that reaches an outcome MUST persist its report; interruptions retain available evidence without inventing completed outputs. A completed proposal report is preliminary and can never authorize a write, even when its checks pass.

```yaml
schema_version: current-runtime-preservation-bridge-report-v2
assessment: proposal | acceptance
admission: Ref
proposal: Ref | null
...
materialized_draft_hash: sha256 | null  # normalized compatibility hash
transformations:
  - id: version_stamp
...
unmapped_output_unit_ids: [string]
pending_obligations:
  - kind: correspondence | deletion | weakening | addition | equivalence | strengthening | supersession
    base_unit_ids: [string]
    output_unit_ids: [string]
    reason: string
failures:
  - category: invalid_binding | invalid_artifact | unauthorized_deleted | ambiguous | disallowed_weakening | allowed_surface_overrun | unauthenticated_supersession | contract_collapse | untrusted_version_edit | unsupported_transform | client_timeout | transient_client_failure | persistence_failure
    ...
    reason: string
validation_result: pass | fail | pending | not_completed
outcome: eligible | awaiting_operator_evidence | rejected | technical_failure
next_action: none | review_proposal | submit_acceptance_request | technical_resume | new_authorized_attempt | inspect_and_repair
```

`assessment` determines the admission schema. A complete comparison requires the proposal, both text Refs, materialized inventory/hash, and exactly one disposition entry per base unit. Report fields copied from the proposal MUST equal it. Evidence uses the scope leaf's explicit correspondence relation; report entries project that relation with finding/evidence references. Original and output IDs MUST resolve in their respective inventories. Final disposition entries follow base byte order; successor, addition and unmapped output lists follow output byte order. Evidence references must be in the acceptance admission; the referenced evidence's projected correspondence/addition entries must equal the report entries they support. Include all detected failures and all identifiable pending obligations, not only the first. Empty affected IDs require an explicit non-unit or unresolvable-identity reason.

Proposal assessment runs every decidable structural, binding, hygiene, surface and limit check. A missing mapping that an operator could explicitly resolve is recorded as `ambiguous` with a `correspondence` pending obligation, not fabricated pairing. Unassigned output units are listed in `unmapped_output_unit_ids`, rather than assumed additions. Known additions are enumerated separately. Mechanical successors, additions and unmapped outputs collectively cover the materialized inventory without overlap. Semantic claims requiring unavailable attestations remain pending; use `ambiguous` for base-unit dispositions awaiting those attestations rather than a successful semantic disposition. Proposal assessment does not invent evidence or certify equivalence. Pending-obligation ID arrays locate work to review; they are not an authoritative pairing relation.

Pending is allowed only when no independent hard failure exists. A known forbidden surface change, invalid ID/reference, conflicting supplied mapping, unsupported transform, corruption or contract collapse remains a failure even if evidence is also missing. If unresolved correspondence prevents a check, record that obligation and withhold `pass`; do not assume the check succeeded. Concrete relocation/deletion/weakening contrary to the approved permissions is not concealed as pending semantic review. Any later newly authorized acceptance re-evaluates these checks under its explicitly selected surface, retaining the original failure.

Acceptance comparison requires complete correspondence and evidence. `pending_obligations` and `unmapped_output_unit_ids` MUST be empty for `pass`. Every output unit must be a matched successor or a listed addition, never both; only attested supersession permits successor sharing. Every changed/added unit needs admitted finding attribution, except exact trusted transform spans. Missing mapping/evidence at submitted acceptance is deterministic rejection: use `ambiguous` for unresolved correspondence/equivalence/addition/strengthening, additionally `unauthenticated_supersession` for unsupported supersession, and the deletion/weakening categories where applicable. Known semantic effects on verbatim retained units require their actual authorized disposition. Approval cannot waive a failed independent gate.

`pass` requires completed checks and zero failures or pending work. `pending` is proposal-only with available immutable output and no independent failure; outcome is `awaiting_operator_evidence`. `fail` yields `rejected` even when pending obligations also exist. `not_completed` cannot claim total coverage. `eligible` requires `pass`, but only an acceptance assessment can lead to an acceptance marker, and ordinary round gates still apply. Invalid/missing report persistence prevents acceptance. No report stage or `next_action` itself grants authority.

Persist immutable `acceptance.json` beside the acceptance admission only after its final report and all ordinary round gates pass. Schema `preservation-bridge-acceptance-v2` contains exactly `schema_version`, `admission: Ref` (acceptance admission), `report: Ref` (acceptance report), `proposal: Ref`, `materialized_draft: Ref`, `materialized_draft_hash: sha256` (normalized), `accepted_noop: boolean`, and `previous_acceptance: Ref | null`. Bind the immediately prior accepted marker; null is allowed only for the recorded original seed. `accepted_noop` is true iff materialized bytes equal base bytes. No post-validation normalization or stamping is allowed.

A later persistence failure MUST NOT rewrite a completed report. When possible, write `terminal_failure.json` beside the relevant admission, using `preservation-bridge-terminal-failure-v1` with `schema_version`, `admission: Ref`, `report: Ref | null`, `category` (`persistence_failure | invalid_binding`), `reason: string`, and `acceptance: Ref | null`. A non-null acceptance permits only checked mirror repair. If failure reporting also fails, return the technical error directly; absence of evidence is never success.

The acceptance marker is the commit point, not a vNext pointer. Enforce exclusive single-writer access for admission and commit and atomic create-if-absent for immutable files. Do not implement competing-candidate selection. Repeated acceptance of an already committed proposal reuses that marker after validating its chain/current authority; it must not allocate another acceptance or duplicate history, including for no-ops. If authority has since advanced, report the historical commit without reinstalling it. Pending mirror/history repair must finish before another admission may commit. Conflicting immutable bytes halt `invalid_binding`; temporary writes are not artifacts.

Run state/status/terminal reporting MUST expose `preservation_bridge` with `mode` (`enforce | legacy_unguarded`), nullable `capability_version`, nullable `latest_proposal: Ref`, nullable `latest_attempt_report: Ref`, nullable `pending_acceptance_admission: Ref`, nullable `accepted: Ref`, nullable `pending_outcome`, and `next_action`. These are derived readback pointers, not additional commit authorities. Effective config describes proposal defaults; a pending/accepted request supplies its own pinned acceptance authorization. Broken evidence in an opted-in root cannot be displayed as legacy. Keep previous failures inspectable and distinguish proposal waiting from accepted state.
