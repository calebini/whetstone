...
def promote_spec_file_for_phase2(*, spec_path: Path, history_path: Path, rounds_dir: Path, config=None) -> VersionPromotionResult:
    ...
    if active(Path(rounds_dir).parent):
        ...
        require(config is not None, 'guarded Phase 2 promotion requires explicit runtime configuration')
        require(spec_path.resolve() == config.spec_path.resolve() and history_path.resolve() == config.history_path.resolve()
                and rounds_dir.resolve() == config.rounds_dir.resolve(), 'Phase 2 promotion paths differ from the guarded root')
        return enter_phase2(Path(rounds_dir).parent,config)
