...
Next: implement step 4 runtime adapters, scheduler/readback updates and downstream consumers on the completed isolated acceptance/repair increment below. No full Phase 1 sharpening run is a prerequisite. Reopen a focused contract review only if implementation exposes a concrete unresolved decision or requires a normative change. Schema publication does not provide runtime enforcement. D6-D8 remain later-vNext gates; the integrated 18.1 implementation/qualification checks remain open.
