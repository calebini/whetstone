...
Step 4 cross-path qualification is complete for the current-runtime developer bridge; the evidence map below records the covered checks. Next: a separate opt-in activation change and its configuration/operator-documentation checks. No full Phase 1 sharpening run is a prerequisite. Reopen a focused contract review only if implementation exposes a concrete unresolved decision or requires a normative change. Schema publication does not provide runtime enforcement. D6-D8 remain later-vNext gates; the integrated 18.1 implementation/qualification checks are evidenced below; public activation remains gated.
...
- [x] Add local `preservation-review`, `preservation-attest`, `preservation-request` and `preservation-accept` commands with explicit effect adoption, automatic line-ID/hash binding, predecessor tracking and read-only dry-run.
...
Horizontal runtime checkpoint (2026-09-06; committed as `b0ce6b0`):
...
- [x] Add Reviewer timeout continuation, review-only closeout/verification and scheduler continuation after local acceptance in the subsequent checkpoint below.
- [x] Finish vertical consolidated/source-review integration and continuation in the subsequent checkpoints below.
- [x] Finish dedicated Phase 2 entry maintenance and inherited authorization in the checkpoint below.
- [x] Finish guarded budget extensions and bounded Phase 2 closeout-existing in the checkpoint below.
- [x] Finish full cross-path qualification before enabling runtime configuration.
...
Phase 1 continuation checkpoint (2026-09-06; committed as `2638124`):

- [x] Reconstruct the original horizontal/focused scheduler, profile debt, effective budgets and round lineage from immutable accepted evidence. Resume after local acceptance without replaying completed rounds; require current-base authorization before the next editing round.
- [x] Freeze Reviewer inputs before invocation; retain typed timeout predecessors and validated feedback. Retry only unchanged Reviewer operations, reuse complete feedback after interrupted canonical writes, and refuse deterministic failures or altered evidence.
- [x] Add review-only closeout receipts bound to the accepted marker and deterministic canonical outputs. Verify after edits without another Editor call or acceptance marker; retain serious findings, recover interrupted completion and keep each completed review's accounting once.
- [x] Expose verified continuation through status and read-only resume planning. Preserve focused completion semantics, refuse false readiness claims, changed scheduler settings and unsolicited closeout rounds, and leave repeated completion read-only.

Validation: `tests/test_preservation_continuation.py` adds 20 scripted regressions; 140 preservation tests and 439 full-suite tests pass. Retained local toy evidence: `rounds/preservation-continuation-checkpoint-002/RESULTS.md` (focused clean verification and interrupted multi-profile closeout with serious residuals). No live model calls. See the [runtime developer guide](docs/PRESERVATION_RUNTIME_DEVELOPER.md). This completes the horizontal Phase 1 continuation checkpoint within step 4. Vertical consolidated editing, Phase 2 maintenance/inherited authorization, guarded budget extensions and complete cross-path qualification remain open; public bridge activation stays unavailable.


Vertical source/consolidation checkpoint (2026-09-06; committed as `9d5b610`):

- [x] Integrate the initial vertical profile sweep with immutable source-review inputs and completion receipts bound to the admitted seed, scope, settings and original profile/round identities.
- [x] Retain the merged Reviewer artifact and every original source review; bind ordered source receipts in frozen configuration. Recompute the existing deterministic merge at admission and acceptance, and apply ordinary issue checks once over that verified merge.
- [x] Route the first consolidated Editor output through proposal assessment and local acceptance/repair before authoritative writes. Retain serious findings from all profiles; allow exact consolidated Editor timeout retry without replaying source reviews.
- [x] Require normal seed no-op acceptance before an all-empty vertical sweep can report stability. Keep repeated completion read-only and stop other accepted first-cycle operations without claiming post-edit verification.
- [x] Finish later vertical cycles, vertical Reviewer recovery and vertical closeout/replay in the subsequent checkpoint below.

Validation: `tests/test_preservation_vertical.py` adds 15 scripted tests. The 453-test full suite passed, followed by a passing exact-byte CRLF regression (454 tests total; 155 preservation tests). Coverage includes source/merge tampering, frozen schema loss, unrelated serious findings, source receipt-write failure, source corruption during/after editing, local acceptance and mirror repair, exact Editor timeout retry, and all-empty seed no-op acceptance. No live model calls. Retained toy evidence: `rounds/preservation-vertical-checkpoint-001/RESULTS.md`. This completes the first vertical source/consolidation checkpoint within step 4, not full vertical scheduling or slice 18.1 qualification. See the [runtime developer guide](docs/PRESERVATION_RUNTIME_DEVELOPER.md).


Vertical continuation/closeout recovery checkpoint (committed as `f6988ae`):

- [x] Reconstruct the original vertical cycle/profile/budget schedule from immutable source and closeout receipts plus accepted consolidations. Replay completed operations through the existing runner without client invocation or duplicate accounting.
- [x] Bind later-cycle sources to the immediately preceding consolidation, preserve unequal profile budgets, and require current-base authorization before another source/edit cycle. Exhausted profiles retain verification debt on changed bytes.
- [x] Resume exact vertical Reviewer timeouts, reuse retained successful feedback after output/receipt-write interruption, and recover committed review receipts before scheduler advancement. Preserve frozen scope, configuration, prompts, timeouts and accepted lineage.
- [x] Recover closeout across profiles without repeating completed reviews, losing earlier serious findings or invoking the Editor. Clean verification completes Phase 1; residuals terminate without a stability claim. Completed continuation remains read-only.

Validation: ten new scripted journeys in `tests/test_preservation_vertical_recovery.py`; 165 preservation tests and 464 full-suite tests pass. Tests cover acceptance-to-closeout, interrupted closeout with serious findings, unequal budgets, current-base refusal, later consolidated editing and local acceptance, source timeout, receipt persistence/advancement faults, exact-byte CRLF closeout recovery, changed retry inputs and repeated completion. Retained toy evidence: `rounds/preservation-vertical-recovery-checkpoint-001/RESULTS.md`. No live models. Guarded budget extensions, Phase 2 maintenance/inherited authorization and complete cross-path qualification remain open; public activation stays unavailable.


Phase 2 maintenance/inherited-authorization checkpoint (committed as `6da1aa4`):

- [x] Admit `phase2_entry` under its dedicated attempt path, with null round/profile/client fields, empty feedback/null summary and a separate empty-evidence acceptance request. Bind fresh inventory, trusted transform and accepted lineage without a numbered round or review-budget charge.
- [x] Reconstruct full Phase 1 handoff eligibility from immutable profile reviews and accepted output. Refuse false mutable stability, pending/corrupt evidence and missing verification. Preserve exact bytes outside trusted version spans, including CRLF.
- [x] Restrict inherited authorization to maintenance over the immediate accepted parent. Keep initial seed no-ops normally admitted; require exact current-base authorization for Editor/supplied output.
- [x] Integrate clean Phase 2 no-op convergence and separately authorized Phase 2 editing/local acceptance through the existing runner and acceptance service. Preserve acceptance authority in state, history, declaration and downstream checks.
- [x] Exercise read-only local acceptance planning and entry recovery before/after commit and after request persistence failure. Reuse the operation with no double stamp, history entry, client call or budget accounting.

Validation: all 478 full-suite tests pass, including 179 preservation tests and 14 new scripted maintenance journeys. The 14-test maintenance suite also passes after the final exact-parent/bound-configuration tightening. Retained toy evidence: `rounds/preservation-maintenance-checkpoint-001/RESULTS.md` (clean convergence and interrupted entry completion). Generic Phase 2 technical continuation remains unsupported. Guarded budget extensions, Phase 2 closeout-existing and full cross-path qualification remain open; public activation stays unavailable. No live model calls.


Budget-extension and bounded Phase 2 closeout checkpoint (working changes based on `6da1aa4`):

- [x] Admit explicit Phase 1 budget grants against completed authority and scheduler evidence. Persist deterministic events before appended clients; retain original recipes and apply verified budget overlays during horizontal/focused and vertical replay.
- [x] Count closeout beyond configured budgets, reopen eligible vertical profiles, preserve current-base authorization and retain serious findings until ordinary acceptance resolves them.
- [x] Recover grant publication, exact interrupted Reviewer/Editor operations and completed feedback without duplicate grants or overwritten attempts. Keep later state writes and acceptance snapshots bound to grant history.
- [x] Admit bounded Phase 2 Reviewer-only closeout against the accepted chain, exact base, frozen settings and ordered missing profiles. Reuse completed reviews, stop on serious findings and recover declaration/terminal publication without Editor calls or generic Phase 2 resume.
- [x] Expose validated extension/closeout guidance while keeping public bridge activation gated.

Validation: all 490 distinct tests are validated across the main regression run and targeted reruns, including 191 preservation tests and 12 new budget/closeout journeys. Four test modules affected by the temporary harness import path passed on normal invocation; a fresh vertical scheduling diagnostic regression was fixed and passed with the extension journey. Final budget-status and terminal-binding rechecks also pass. Retained evidence and raw validation logs: `rounds/preservation-budget-closeout-checkpoint-001/RESULTS.md` and `results.json`. Full cross-path qualification remains the next checkpoint; this implementation does not enable public bridge configuration. No live model calls.


Full cross-path qualification checkpoint (working changes based on `6da1aa4`):

- [x] Join normal/supplied/focused/horizontal/vertical execution, local acceptance, Phase 1 continuation, grants/retries, Phase 2 entry/rounds, bounded closeout, declaration and dry/live consumers through the existing preservation service.
- [x] Complete seven positive line-based operator-effect journeys, recording displayed effects, mappings, interaction counts and zero acceptance-stage Editor calls; exercise the actual CLI acceptance/status/resume path.
- [x] Reject padded frozen schema loss, false convergence, altered declarations/rubric identity and concurrent source changes. Keep clean profiles bound to current accepted bytes after a later horizontal mutation; reconstruct the same invalidation on replay.
- [x] Verify exact-byte apply-back, complete Phase 2 verification/terminal receipt consumption, and successful retry without duplicate calls, markers, stamps, history or accounting.
- [ ] Enable public opt-in configuration in a separate release change; `PRESERVATION_BRIDGE_QUALIFIED` remains false.

Validation: All 507 distinct tests pass: a clean 504-test full discovery run plus three qualification tests added after that discovery; the strengthened padded-loss assertion also passes its final recheck. This includes 208 preservation tests. The 32-test focused runtime/consumer suite and ten retained journeys pass. No live model calls. Exact criteria-to-test mapping and limits: [preservation qualification](docs/PRESERVATION_QUALIFICATION.md). Retained evidence: `rounds/preservation-qualification-001/RESULTS.md`, `results.json` and `validation.json`. There is no unresolved normative decision from this checkpoint. Public activation, live-model reliability and vNext are outside this qualification result.

Implementation checks:

- [x] Publish the revised bridge contracts: surface/inventory, proposal admission/record, evidence-v2 with explicit correspondence, acceptance request/admission, report-v2, acceptance-v2 marker and terminal-failure contract. Validate base/scope/surface/findings before proposal generation; collect and bind exact-effect evidence after output exists. Reject superseded draft shapes and unknown fields. Verify the artifact graph is acyclic.
- [x] Build the versioned structural section/unit inventory and deterministic comparison using heading path, unit type and defined parent position/ordinal. Cover preamble, parent bodies, normative statements, schemas/fields, tables/rows, enums, flags, artifacts, states, scenarios and references, including nesting/overlap. Exclude fenced pseudo-headings, preserve unauthorized units by default, require total base-unit disposition and separately enumerate additions. Unresolved correspondence is pending during proposal assessment when no hard failure exists, and blocks final acceptance; explicit adopted mappings must resolve it without fuzzy inference.
- [x] Persist raw proposal bytes and materialize separate proposed output through the defined Orchestrator transformations. Bind both exact-byte hashes and the ordered transformation evidence; run preservation on the final materialized bytes before any authoritative writes or accepted history. Keep normalized lineage hashes separately labeled. Freeze effective config before execution and retain origin-specific Reviewer feedback/Editor summary references; acceptance cannot substitute later defaults, new resolution claims or different normal-round evidence. Changed transformation inputs require a new proposal, never post-approval restamping.
...
Next action: make the separate 18.1 public opt-in activation change, with configuration acceptance/refusal tests and operator documentation reflecting the qualified current-runtime paths. The integrated qualification checks now have executable evidence; see [the qualification map](docs/PRESERVATION_QUALIFICATION.md) and the retained 507-test accounting. Keep `PRESERVATION_BRIDGE_QUALIFIED` false until that release change is made. Generic Phase 2 technical resume remains unsupported, and older experimental guarded Phase 2 roots without the new frozen rubric binding cannot acquire retrospective convergence proof. The bounded audit is complete; no full Phase 1 sharpening run is an implementation prerequisite. No unresolved normative decision emerged from qualification. Any additional nested live-client audit or usability trial requires its own payload authorization. D6-D8 and later vNext work retain their separate prerequisites.
