...
The live bridge remains unavailable. Its schema/inventory foundations, immutable proposal capture, materialization and preliminary reports now have [executable tests](../../../contracts/PRESERVATION_BRIDGE.md); isolated acceptance/repair now has executable journeys, while runtime integration remains pending. The proposal tests demonstrate clarification pending, hidden frozen field/table/enum loss rejected, and unchanged preservation eligible, while retaining the authoritative spec unchanged. These are source texts and test-design instructions. Generate actual proposal/evidence/request artifacts before using the [isolated local acceptance commands](../../../docs/PRESERVATION_ACCEPTANCE_DEVELOPER.md). `cases.json` is a local fixture catalog, not a public Whetstone schema. Do not submit it as a scope contract, finding, surface, proposal, evidence or acceptance request. The implementation must generate and validate those artifacts using the actual supported contracts.
